...
- **Database:** PostgreSQL (Centralized storage for users and WA sessions).
...
- `users`: Stores account info (`username`, `email`, `password`, `is_verified`, `verification_token`, `system_prompt`, `message_count`).
- `wa_sessions`: Managed by `whatsmeow/sqlstore` inside the same PostgreSQL database to store WhatsApp session blobs.
...
5. **Usage Tracking:** Every message sent (AI or Manual) is counted in the `message_count` column for each user.
6. **Webhook:** Every AI response triggers a POST request to a configured `WEBHOOK_URL` for external monitoring/logging.
7. **Rate Limiting:** Protects endpoints from brute force and spam (Global: 30 requests/min).
